...
const AdminNominations = () => {
  ...
  useEffect(() => {
    document.title = 'Administrate nominations';
  }, []);
  ...
  const defaultParams = {
    ...
    sort: params.get('sort')
      ? params.get('sort')
      : 'owner_last_name,owner_first_name,lower(vessel_name),imo,readable_nomination_status,window_start,window_end,id',
    search: params.get('search') ? params.get('search') : '',
  };
  const [search, setSearch] = useState(params.get('search') ? params.get('search') : '');
  const { loading, data, error, fetchData } = useApi('get', 'nominations', defaultParams);
  const { loading: berthsLoading, data: berthsData, error: berthsError } = useApi('get', 'all-nominatable-berths', {});
  const { data: consigneeData, error: consigneeError } = useApi('get', 'consignee-users', {});
  ...
  nominations.forEach(p => {
    ...
    p.owner = p.owner_last_name + ', ' + p.owner_first_name;
  });
  ...
  const fetchedConsigneeUsers = consigneeError || !consigneeData ? [] : consigneeData;
  counter = 1;
  fetchedConsigneeUsers.forEach(p => {
    p._row = counter;
    counter++;
    p.key = p.id;
    p.label = p.name;
  ...
  const defaultOwner = fetchedConsigneeUsers.length === 0 ? '-1' : fetchedConsigneeUsers[0].id;
  ...
  const showModal = async id => {
    ...
    try {
      const { data } = await apiCall('get', 'nomination-by-id', { id });
  ...
  const handleColumnClick = async id => {
    ...
    if (id === 'vessel_name') {
      ...
      sort =
        'lower(vessel_name),owner_last_name,owner_first_name,imo,readable_nomination_status,window_start,window_end,id';
    } else if (id === 'imo') {
      sort =
        'imo,owner_last_name,owner_first_name,lower(vessel_name),readable_nomination_status,window_start,window_end,id';
    } else if (id === 'readable_nomination_status') {
      sort =
        'readable_nomination_status,owner_last_name,owner_first_name,lower(vessel_name),imo,window_start,window_end,id';
    } else if (id === 'window_start') {
      sort =
        'window_start,owner_last_name,owner_first_name,lower(vessel_name),imo,readable_nomination_status,window_end,id';
    } else if (id === 'window_end') {
      sort =
        'window_end,owner_last_name,owner_first_name,lower(vessel_name),imo,readable_nomination_status,window_start,id';
    } else if (id === 'id') {
      sort =
        'id,owner_last_name,owner_first_name,lower(vessel_name),imo,readable_nomination_status,window_start,window_end';
    } else if (id === 'owner') {
      id = 'owner_last_name';
      sort =
        'owner_last_name,owner_first_name,lower(vessel_name),imo,readable_nomination_status,window_start,window_end,id';
  ...
  const handleSave = async values => {
    ...
    let berth_ids = [];
    let consignee_user_id = -1;
    if (owner === '-1') {
      consignee_user_id = defaultOwner;
    } else {
      consignee_user_id = owner;
    }
    if (first_berth !== '0') {
    ...
    setApiCallPending(true);
    try {
      await apiCall('post', 'nomination-for-consignee', {
        consignee_user_id,
        company_name,
        email,
        imo,
  ...
  const handleDelete = async id => {
    setApiCallPending(true);
    try {
      await apiCall('delete', 'nominations', { id });
    } catch (e) {
      setApiCallPending(false);
      throw e;
    }
  ...
  return (
    <Layout>
      ...
      <Page fullWidth title={t('Adminstrate nominations')}>
